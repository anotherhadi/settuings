...
import (
	...
	settuings "github.com/anotherhadi/settuings"
...
func readDoc(name string) string {
	b, _ := settuings.DocsFS.ReadFile("docs/" + name)
	return string(b)
}

var contentMarkdown = readDoc("about.md")
...
func (m *Model) applySearch() {
	...
	if len(m.matches) > 0 {
		util.SetYOffset(&m.viewport, m.matches[0].line)
	}
}
...
func (m *Model) searchNext() {
	...
	m.rebuildViewportContent()
	util.SetYOffset(&m.viewport, m.matches[m.matchIndex].line)
}
...
func (m *Model) renderStatusBar() string {
	...
	return lipgloss.NewStyle().Padding(0, 1).Render(m.help.View(aboutKeyMap{width: m.width}))
}

type aboutKeyMap struct{ width int }

func (aboutKeyMap) ShortHelp() []key.Binding {
	g := keys.Keys.Global
	d := keys.Keys.Docs
	return []key.Binding{g.Up, g.Down, d.Search, g.Help}
}

func (m aboutKeyMap) FullHelp() [][]key.Binding {
	g := keys.Keys.Global
	d := keys.Keys.Docs
